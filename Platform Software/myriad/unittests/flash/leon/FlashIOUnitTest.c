...
static void testFlashMemoryIsEmpty( void ) {
    ...
    TEST_ASSERT_EQUAL_INT( 8253440/*8MB-(64KB*2+4KB)*/,vailableMemory );
}
...
static void testAvailableSpace( void ) {
    ...
    TEST_ASSERT_EQUAL_INT( 8253440/*8MByte-(64KBytes*2+4KBytes)*/,
                           availableSpace );

    FlashFileWrite( fileHandler, loremIpsum, 128 );
    availableSpace = FlashFileGetAvailableSpace( fileHandler );
    TEST_ASSERT_EQUAL_INT( 8253312/*8MByte-(64KBytes*2+4KBytes+128Bytes)*/,
                           availableSpace );
    // close file handler
    closed = FlashFileClose( &fileHandler );
    TEST_ASSERT( true == closed );
    ...
    availableSpace = FlashFileGetAvailableSpace( fileHandler );
    TEST_ASSERT_EQUAL_INT( 8245248/*8MByte-(64KBytes*2+4KBytes+2*4KBytes)*/,
                           availableSpace );

    FlashFileWrite( fileHandler, loremIpsum, 128 );
    availableSpace = FlashFileGetAvailableSpace( fileHandler );
    TEST_ASSERT_EQUAL_INT(
            8245120/*8MByte-(64KBytes*2+4KBytes+2*4KBytes+128Bytes)*/,
            availableSpace );
    // close file handler
    closed = FlashFileClose( &fileHandler );
    TEST_ASSERT( true == closed );
