 ...
 * The EoT MQTT Client.
 */
public class EoT_Python_MQTT_Client implements MqttCallback {
    ...
    public final String topicEOTShowImage = "EOTShowImage";
    public final String topicEOTPython = "EOTPython";

    // Private instance variables
    private MqttClient client;
    private MqttConnectOptions conOpt;
    private EoT_Python_MainFrame mainFrame;
     ...
     */
    public EoT_Python_MQTT_Client(String brokerip, int brokerport) {
     ...
     * @param qos the quality of service to delivery the message at (0,1,2) (0
     * in this case)
    ...
    public void messageArrived(String topic, MqttMessage messageArrived) throws Exception {
        ...
        switch (topic) {
            ...
            case (topicEOTUploadFileSD):
                ...
                break;
            case (topicEOTPython):
                // Receives the result of uploading a file to Python
                if (this.operationDone == false) {
                    result = Integer.parseInt(new String(messageArrived.getPayload()));
                    if (result == 2) {
                        this.send_next = 1;
                    } else {
     ...
     * @param frame
     */
    public void setMainFrame(EoT_Python_MainFrame frame) {
        mainFrame = frame;
    }
    ...
    /**
     * Sends a message in the topicSnapshot topic to get the image from the
     * broker
     *
     * @throws MqttException
     */
    public ImageIcon requestSnapshot() throws MqttException {
    ...
    public void setSnapshot(ImageIcon image) {
        this.mainFrame.setSnapshot(image);
        //this.repaint();
    }

    public void requestShowImage() throws MqttException, InterruptedException {
        // request for the image to show
        //this.operationDone = false;
        publish(this.topicEOTShowImage, this.qos, this.message.getBytes());
        try {
            Thread.sleep(100);
        } catch (InterruptedException ex) {
            Logger.getLogger(EoT_Python_MainFrame.class.getName()).log(Level.SEVERE, null, ex);
        }
        subscribe(this.topicEOTShowImage, this.qos);
        //sampleClient.unsubscribe(topicSnapshot);
        //while (!this.operationDone) {
        //    Thread.sleep(100);
        //}
    ...
    void sendCode(String eotPython, int i, byte[] data, boolean disableOutputs) throws MqttException {
        this.send_next = 1;
        this.operationDone = false;
        try {
            requestShowImage();
        } catch (InterruptedException ex) {
            Logger.getLogger(EoT_Python_MQTT_Client.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        //message 1 (buffer length)
        String disableOut = " 0";
        if(disableOutputs) disableOut = " 1";
        MqttMessage message2 = null;
        if ((data.length % total_length) == 0) {
            message2 = new MqttMessage((Integer.toString(data.length / total_length) + disableOut).getBytes());
        } else {
            message2 = new MqttMessage((Integer.toString(data.length / total_length + 1) + disableOut).getBytes());
        }
        message2.setQos(qos);

        client.publish(eotPython, message2);

        //int sleep_time = 100;
        int length_sent = 0;
        int actual_packet_sent = 0;
        while (length_sent < data.length) {
            while (this.send_next == 0);
            this.send_next = 0;
            //Thread.sleep(sleep_time); //sleep 0.1 sec (10KB/s)
            //add number of package (6 characters)
            ...
            message1 = new MqttMessage(subData);
            message1.setQos(qos);
            client.publish(eotPython, message1);
            length_sent += total_length;
